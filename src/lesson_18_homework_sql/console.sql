...
/* выведем имена и id объединив две таблицы humans*/
select data_human_hobby.id_human, humans.name
from data_human_hobby
inner join humans on data_human_hobby.id_human = humans.id_user;

--выведем имена людей и количество хобби
select humans.name, count(
    id_hobby
    )
from data_human_hobby
inner join   humans on data_human_hobby.id_human = humans.id_user
...
from data_human_hobby
         inner join   humans on data_human_hobby.id_human = humans.id_user
group by humans.name having count(id_hobby) > 1;
